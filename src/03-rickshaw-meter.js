...
export function parseFare(fareString) {
  ...
  const parsed = parseFloat(fareString.trim());
...
export function getDistanceDifference(from, to) {
  // Your code here
  let varFrom = parseInt(from);
  let varTo = parseInt(to);
  if (Number.isNaN(varFrom) === true || Number.isNaN(varTo) === true) return -1;
  let diff = varFrom - varTo;
