...
export function findPassenger(passengers, name) {
  ...
  return passengers.find(
    (p) => typeof p.name === "string" && p.name.toLowerCase() === searchName,
  );
}
...
export function isAnyWaitlisted(passengers) {
  ...
  return passengers.some((s) => s.status === "waitlisted");
}

export function areAllConfirmed(passengers) {
  // Your code here
  if (Array.isArray(passengers) === false || passengers.length === 0)
    return false;
...
export function getWaitlistedPassengers(passengers) {
  ...
  return passengers.filter((s) => s.status === "waitlisted");
}
